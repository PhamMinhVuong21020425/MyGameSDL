...
TileMap::TileMap(SDL_Renderer* &gRenderer)
{
    ...
    //defenceSideTexture = loadTexture("image/white.png", gRenderer);

    fireTexture = loadTexture("image/fire.png", gRenderer);
}
...
void TileMap::drawMap(SDL_Renderer* &gRenderer)
{
    ...
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            if (tile[i][j] == 2)
            {
                renderTiles(defenceSideTexture, j * coefficient, i * coefficient, &desRect, gRenderer );
            }
            else if(tile[i][j] == 9)
            {
                //renderTiles(defenceSideTexture, j * coefficient, i * coefficient, &desRect, gRenderer );
                renderTiles(fireTexture, j * coefficient, i * coefficient, &desRect, gRenderer );
            }
        }
    }
}
...
TileMap::~TileMap()
{
    ...
    SDL_DestroyTexture(defenceSideTexture);
    defenceSideTexture = NULL;
